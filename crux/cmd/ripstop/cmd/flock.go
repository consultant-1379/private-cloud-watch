...
var flockCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		clog.Log.SetDebug()
		vip := parseCmd(cmd)
		msg := fmt.Sprintf("Ripstop Bootstrap Args: port=%d key=%s name=%s ip=%s beacon=%s horde=%s networks=%s visitor=%t fire=%d wait=%t",
			vip.GetInt("port"),
			vip.GetString("key"),
			vip.GetString("name"),
			vip.GetString("ip"),
		...
		ruck.BootstrapRipstop(vip.GetInt("port"), vip.GetString("key"), vip.GetString("name"), vip.GetString("ip"), vip.GetString("beacon"),
			vip.GetString("horde"), vip.GetString("networks"), vip.GetString("certdir"), vip.GetBool("visitor"))
		time.Sleep(10 * time.Second)
		fmt.Printf("Ripstop completed!!\n")
	},
}
...
func init() {
	...
	flockCmd.Flags().String("horde", "sharks", "horde name")
