...
_init() {
    ...
    GCC_VERSION="4.0"
    LLVM_VERSION="7.0.0"
    # YASM_VERSION="1.2.0" - FIXME(skaar): do we care?
    GIT_VERSION="1.0"
    GO_VERSIONS="1.11.5 1.11.6 1.12.1 1.12.6"
    OSX_VERSION="10.8"
    UNAME=$(uname -sm)
...
check_deps() {
    local installed_go="$(env go version 2>/dev/null | sed 's/^.* go\([0-9.]*\).*$/\1/')"
    echo -e "\n>>> Go version: $installed_go"
    {
        SAVEIFS=$IFS
        unset IFS
        local found=0 versions=($GO_VERSIONS)
        local len=${#versions[@]}
        for ((i=0; i<$len; i++)); do
            if [[ "${versions[i]}" == "$installed_go" ]]; then
                found=1
            fi
        done
        if [[ $found -eq 0 ]]; then
            MISSING="${MISSING} golang(${versions[$len-1]})"
        fi
        IFS=$SAVEIFS
    }
...
main() {
    ...
    echo -n "Checking dependencies for Stix... "
    check_deps
    ...
    if [ "x${MISSING}" != "x" ]; then
        ...
        echo "${MISSING}"
        echo
        echo "INSTALLGO.md for further instructions"
        exit 1
    fi
    echo "Done"
}
