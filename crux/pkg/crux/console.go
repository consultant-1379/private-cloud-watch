...
// Exit - exit, only once, after tidying up
func Exit(rc int) {
	mutex.Lock()
	wuzExiting := exiting
	GetLogger().Log(nil, "crux.Exit: wuz=%v", wuzExiting)
	exiting = true
	mutex.Unlock()
	if wuzExiting { // only one thread may os.Exit()
		var nilChan chan bool
		GetLogger().Log(nil, "crux.Exit: waiting...")
		<- nilChan
	}
	GetLogger().Log(nil, "crux.Exit: walrus.Exit...")
	walrus.Exit(rc)
}
