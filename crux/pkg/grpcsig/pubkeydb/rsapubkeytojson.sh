...
if [ $# -le 1 ]
then
	echo 'rsa [public key file] AND [service] string must be specified, e.g.: ~/.ssh/id_rsa.pub "plogiston-server"'
...
then
	USER_NAME=${3%}
	echo '{"service":"'${SERVICE}'","name":"'${USER_NAME}'","keyid":"/'${SERVICE}'/'${USER_NAME}'/keys/'${FINGERPRINT:9:47}'","pubkey":"'`cat ${PUBKEY_PATH}`'","stateadded":-1}'
	exit 0
fi
