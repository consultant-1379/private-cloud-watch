...
func (h *Handler) Run(ctx context.Context) error {
	...
	if (h.options.ServerCertificate == "") != (h.options.ServerKey == "") {
		return fmt.Errorf("--web.server.certificate and --web.server.key must both be specified")
	}
	if (h.options.ClientCAs != "") && (h.options.ServerCertificate == "") {
		return fmt.Errorf("--web.server.clientcas requires --web.server.certificate and .key")
	}
	...
	if h.options.ServerCertificate != "" {
		cert, err := x509ca.ReadCertPEMFile(h.options.ServerCertificate)
		if err != nil {
			return err
		}
		priv, err := x509ca.ReadKeyPEMFile(h.options.ServerKey)
		if err != nil {
			return err
		}
		...
		tlsConfig := &tls.Config{
			Certificates:     []tls.Certificate{tlsLeaf},
			CurvePreferences: []tls.CurveID{tls.X25519},
		}
		if h.options.ClientCAs != "" {
			tlsConfig.ClientAuth = tls.RequireAndVerifyClientCert
			tlsConfig.ClientCAs = x509.NewCertPool()
			pems, err := ioutil.ReadFile(h.options.ClientCAs)
			if err != nil {
				return err
			}
			if !tlsConfig.ClientCAs.AppendCertsFromPEM(pems) {
				return fmt.Errorf("%s: no client CAs found", h.options.ClientCAs)
			}
		}
