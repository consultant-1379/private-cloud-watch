...
func NewRoundTripperFromConfig(cfg HTTPClientConfig, name string) (http.RoundTripper, error) {
	return NewRoundTripperFromConfigWithDialer(cfg, name, new(net.Dialer))
}

// NewRoundTripperFromConfigWithDialer returns a new HTTP RoundTripper configured for the
// given config.HTTPClientConfig. The name is used as go-conntrack metric label.
func NewRoundTripperFromConfigWithDialer(cfg HTTPClientConfig, name string, dialer *net.Dialer) (http.RoundTripper, error) {
	newRT := func(tlsConfig *tls.Config) (http.RoundTripper, error) {
		...
		var rt http.RoundTripper = &http.Transport{
			...
			DialTLS:             tlsConfig.DialWithDialerFn(dialer),
	...
	return newTLSRoundTripper(tlsConfig, cfg.TLSConfig.CAFile, cfg.TLSConfig.CAFileIsTar, newRT)
}
...
func NewTLSConfig(cfg *TLSConfig) (*tls.Config, error) {
	if cfg.CAFileIsTar {
		tlsConfig, err := x509ca.TLSConfigTar(cfg.CAFile)
		if err != nil {
			return nil, err
		}
...
type TLSConfig struct {
	...
	// Follow crux certificate conventions.
	CAFileIsTar bool `yaml:"ca_file_is_tar"`
}
...
func (t *tlsRoundTripper) RoundTrip(req *http.Request) (*http.Response, error) {
	...
	if t.caFileIsTar {
		newConfig, err :=  x509ca.TLSConfigTarBytes(b)
		if err != nil {
			return nil, fmt.Errorf("unable to use specified CA tar %s", t.caFile)
		}
		tlsConfig.RootCAs = newConfig.RootCAs
		tlsConfig.GetClientCertificate = newConfig.GetClientCertificate
	} else	if !updateRootCA(tlsConfig, b) {
