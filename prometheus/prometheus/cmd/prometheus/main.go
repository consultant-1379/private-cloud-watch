...
func main() {
	...
	a.Flag("web.server.certificate", "Path to server certificate.").
		Default("").StringVar(&cfg.web.ServerCertificate)

	a.Flag("web.server.key", "Path to server private key.").
		Default("").StringVar(&cfg.web.ServerKey)

	a.Flag("web.server.clientcas", "Path to client CAs (PEM bundle).").
		Default("").StringVar(&cfg.web.ClientCAs)
