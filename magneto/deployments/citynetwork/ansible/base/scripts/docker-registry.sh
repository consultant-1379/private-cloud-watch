...
# set up cert flags
CERT="--key /etc/docker/certs.d/$REG/client.key --cert /etc/docker/certs.d/$REG/client.cert"
...
case "$ACTION" in
    list)
        if [ $# -eq 1 ]; then
            repo=${1}
            if [ -n "$repo" ]; then
                $curl -s $PROTO$REG/v2/$repo/tags/list | jq -r '.tags|.[]'
            fi
        else
            $curl -s $PROTO$REG/v2/_catalog | jq -r '.repositories|.[]'
        fi
    ...
    delete)
        repo=$1
        tag=$2
        digest=$($curl -v -s -H "Accept:application/vnd.docker.distribution.manifest.v2+json" $PROTO$REG/v2/$repo/manifests/$tag 2>&1 |grep "< Docker-Content-Digest:"|awk '{print $3}')
        digest=${digest//[$'\t\r\n']}
        echo "DIGEST: $digest"
